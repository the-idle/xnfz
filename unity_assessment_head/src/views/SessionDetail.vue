<template>
      ...
      <div v-if="detail" style="margin-top: 20px;">
        ...
        <div v-else class="question-list">
          <el-card v-for="(log, index) in logs" :key="index" class="question-card" shadow="hover">
            <template #header>
              <div class="q-header">
                <span class="q-index">第 {{ index + 1 }} 题 <span style="font-size: 12px; font-weight: normal; color: #999;">(ID: {{ log.question_id }})</span></span>
            ...
            <div class="q-body">
              <!-- 后端新接口暂未返回 question 详情，这里做降级显示 -->
              <div v-if="log.question" class="q-prompt">
                {{ log.question.prompt }}
              </div>
              <div v-else class="q-prompt-fallback">
                <el-text type="info">题目详情数据未包含在本次响应中</el-text>
              </div>
  
              <!-- 选项显示 (如果有 question 详情) -->
              <div class="q-options" v-if="log.question && log.question.options">
                <div 
                  ...
                  class="option-item"
                  :class="{
                    'user-selected': log.selected_option_ids.includes(opt.id),
                    'is-correct': opt.is_correct
                  }"
                >
                  <el-icon v-if="opt.is_correct" color="#67C23A" class="opt-icon"><Check /></el-icon>
                  <el-icon v-else-if="log.selected_option_ids.includes(opt.id)" color="#F56C6C" class="opt-icon"><Close /></el-icon>
              ...
              <div style="margin-top: 5px; font-size: 12px; color: #999;">
                答题时间: {{ new Date(log.answered_at).toLocaleString() }}
              </div>
            </div>
          </el-card>
        </div>
  ...
  const logs = computed(() => {
    ...
    return detail.value.answer_logs || [];
  });
  ...
  const fetchDetail = async () => {
    ...
    try {
      // 调用后端新接口
      const res = await request.get<any, any>(`/assessment-results/${sessionId}`);
      detail.value = res;
    } catch (e) {
      console.error(e);
    } finally {
